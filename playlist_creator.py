...
class PlaylistCreator:
    ...
    def make_blend(self, friend: str, friends_playlist_name: str, my_playlist_name: str, limit: int):
        ...
        my_playlist = self.get_playlist_by_name(self.sp.me()['id'], my_playlist_name)
        ...
        result_playlist = self.create_playlist(f"blend_{str(date.today())}_{random.randint(0, 100)}",
                                               f"Generated by yt2sp for {friend} and me",
...
def main():
    ...
    match args.command:
        ...
        case 'get_recommendations':
            start_id = random.randint(0, len(top_tracks_ids)-5)
            result = playlist_creator.get_recommendations(seed_tracks=top_tracks_ids[start_id:start_id+4],
            ...
            recommendations_playlist = playlist_creator.create_playlist(f"recommendations_{str(date.today())}",
                                                                        f"Generated by yt2sp based on my current top{start_id}-top{start_id+4}",
                                                                        tracks_ids)
