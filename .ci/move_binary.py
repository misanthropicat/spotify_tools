...
commit_subject = (
    subprocess.check_output(["git", "log", "-1", "--pretty=format:%s"]).decode("utf-8").strip()
)
...
if is_tag:
    new_commit_message = f'Add binary for {filename_split[1]} {commit_hash}: "{commit_subject}"'
elif is_pr:
    ...
    new_commit_message = f'Add binary for #{pr_number} {commit_hash}: "{commit_subject}"'
else:
